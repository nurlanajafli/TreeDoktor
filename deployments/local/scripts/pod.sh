...
configure_services() {
  wait
  logger "[POD] Configuring services";
  # get the chrome service address
  # aws configure set default.region us-west-2 && \
  # OPS=$(aws ssm get-parameter --name "/ops/info/ip" \
  #                       --query "[Parameter][*][Value]" \
  #                       --output text) && \
  # aws configure set default.region $REGION_ZONE && \
  # sed '/chrome/d' /etc/hosts > /etc/hosts.clean && \
  # sed '$ a $OPS chrome' /etc/hosts.clean > /etc/hosts;
  # configure nginx
  $(process_file "/opt/docker/etc/nginx/vhost.common.d/s3.conf") || { logger '[POD] Failed to process file'; exit 1; }
  wait
}
...
start_services() {
  logger "[POD] Starting sservices";
  supervisorctl start nginxd pm2d queue;
  if [ $? -ne 0 ]; then
...
restart_services() {
  ...
  supervisorctl restart nginxd pm2d queue;
  if [ $? -ne 0 ]; then
...
stop_services() {
  ...
  supervisorctl stop nginxd pm2d queue;
  if [ $? -ne 0 ]; then
...
init() {
  ...
  # configure nginx s3 and retrieve chrome address
  $(configure_services) || { logger '[POD Init] Failed while configuring services'; exit 1; }
  # get and install configs
