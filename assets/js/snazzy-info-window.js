    ...
    var SnazzyInfoWindow = function (_google$maps$OverlayV) {
        ...
        _createClass(SnazzyInfoWindow, [{
            ...
            value: function draw() {
                ...
                if (this._opts.border) {
                    ...
                    if (this._opts.pointer) {
                        ...
                        this._html.pointerBg.style.borderWidth = '5px'//pLength.value - triangleDiff + pLength.units;
                ...
                if (divPixel) {
                    var child = $(this._html.floatWrapper).find('.si-has-border:first');
                    var plusY = 0;
                    var plusX = 0;

                    if(this._opts.placement == 'bottom')
                        var plusY = -5;
                    if(this._opts.placement == 'top')
                        var plusY = 5;
                    if(this._opts.placement == 'right')
                        var plusX = -2;
                    if(this._opts.placement == 'left')
                        var plusX = 2;

                    this._html.floatWrapper.style.top = (Math.floor(divPixel.y) + plusY) + 'px';
                    this._html.floatWrapper.style.left = (Math.floor(divPixel.x) + plusX) + 'px';
                    this._html.floatWrapper.style.height = $(child).height() + 'px';
                    this._html.floatWrapper.style.width = $(child).width() + 'px';
                }
                if (!this._isOpen) {
                    this._isOpen = true;
                    this.resize();
            ...
            value: function onAdd() {
                    ...
                    if (element && args) {
                        for (var i = 0; i < args.length; i++) {
                            ...
                            if (className) {
                                ...
                                /*if(!(args.indexOf('pointer') + 1))
                                    element.className += ' ' + _classPrefix.replace('-', '');*/
                            }
                        }
                    }
                };
                ...
                this._opts.placement = 'top';
                //sleep(500);
                var divPixel = this.getProjection().fromLatLngToDivPixel(this._position || this._marker.position);
                

                /*var elem1 = document.elementFromPoint(divPixel.x - 17, divPixel.y - 35);
                var elem2 = document.elementFromPoint(divPixel.x + 11, divPixel.y - 35);
                var elem3 = document.elementFromPoint(divPixel.x - 17, divPixel.y - 22);
                var elem4 = document.elementFromPoint(divPixel.x + 11, divPixel.y - 22);*/
                var minusY = 0;
                if($(window).width() < 1200)
                    minusY = 20;

                var elem1 = document.elementFromPoint(divPixel.x - 20, divPixel.y - 45 + minusY);
                var elem2 = document.elementFromPoint(divPixel.x + 18, divPixel.y - 45 + minusY);
                var elem3 = document.elementFromPoint(divPixel.x - 20, divPixel.y - 67 + minusY);
                var elem4 = document.elementFromPoint(divPixel.x + 18, divPixel.y - 67 + minusY);

               /* $('body').append('<div id="test"></div>')
                $('body').append('<div id="test1"></div>')
                $('body').append('<div id="test2"></div>')
                $('body').append('<div id="test3"></div>')
                $('#test').css({left:(divPixel.x - 20)+'px', top:(divPixel.y - 25 + minusY)+'px', width:'2px', height:'2px', background:'#000000', position: 'absolute'});
                $('#test1').css({left:(divPixel.x + 18)+'px', top:(divPixel.y - 25 + minusY)+'px', width:'2px', height:'2px', background:'#000000', position: 'absolute'});
                $('#test2').css({left:(divPixel.x - 20)+'px', top:(divPixel.y - 47 + minusY)+'px', width:'2px', height:'2px', background:'#000000', position: 'absolute'});
                $('#test3').css({left:(divPixel.x + 18)+'px', top:(divPixel.y - 47 + minusY)+'px', width:'2px', height:'2px', background:'#000000', position: 'absolute'});*/
                //console.log(this, elem1, elem2, elem3, elem4);

                if($(elem1).is('.si-t') || $(elem2).is('.si-t') || $(elem3).is('.si-t') || $(elem4).is('.si-t')) {
                    this._opts.placement = 'right';

                    minusY = 0;
                    if($(window).width() < 1200)
                        minusY = 10;

                    var elem1 = document.elementFromPoint(divPixel.x + 17, divPixel.y - 32 + minusY);
                    var elem2 = document.elementFromPoint(divPixel.x + 17, divPixel.y - 10 + minusY);
                    var elem3 = document.elementFromPoint(divPixel.x + 57, divPixel.y - 32 + minusY);
                    var elem4 = document.elementFromPoint(divPixel.x + 57, divPixel.y - 10 + minusY);
                    ...
                    $('body').append('<div id="test2"></div>')
                    $('body').append('<div id="test3"></div>')
                    $('#test').css({left:(divPixel.x + 17)+'px', top:(divPixel.y - 32 + minusY)+'px', width:'2px', height:'2px', background:'#000000', position: 'absolute'});
                    $('#test1').css({left:(divPixel.x + 17)+'px', top:(divPixel.y - 10 + minusY)+'px', width:'2px', height:'2px', background:'#000000', position: 'absolute'});
                    $('#test2').css({left:(divPixel.x + 57)+'px', top:(divPixel.y - 32 + minusY)+'px', width:'2px', height:'2px', background:'#000000', position: 'absolute'});
                    $('#test3').css({left:(divPixel.x + 57)+'px', top:(divPixel.y - 10 + minusY)+'px', width:'2px', height:'2px', background:'#000000', position: 'absolute'});*/
                    
                    if($(elem1).is('.si-t') || $(elem2).is('.si-t') || $(elem3).is('.si-t') || $(elem4).is('.si-t')) {
                        this._opts.placement = 'bottom';

                        var elem1 = document.elementFromPoint(divPixel.x - 20, divPixel.y + 15);
                        var elem2 = document.elementFromPoint(divPixel.x + 18, divPixel.y + 15);
                        var elem3 = document.elementFromPoint(divPixel.x - 20, divPixel.y + 35);
                        var elem4 = document.elementFromPoint(divPixel.x + 18, divPixel.y + 35);

                        //console.log(this, elem1, elem2, elem3, elem4);

                        if($(elem1).is('.si-t') || $(elem2).is('.si-t') || $(elem3).is('.si-t') || $(elem4).is('.si-t')) {
                            this._opts.placement = 'left';
                        }
                    }
                }
                ...
                console.log('rigth', divPixel);

                var elem1 = document.elementFromPoint(divPixel.x, divPixel.y);
                var elem2 = document.elementFromPoint(divPixel.x + 90, divPixel.y);
                var elem3 = document.elementFromPoint(divPixel.x, divPixel.y + 54);
                var elem4 = document.elementFromPoint(divPixel.x + 90, divPixel.y + 54);

                if($(elem1).is('.si') || $(elem2).is('.si') || $(elem3).is('.si') || $(elem4).is('.si')) {
                    this._opts.placement = 'bottom';
                }*/
                ...
                // 2. Create the shadow
                if (this._opts.shadow) {
                    this._html.shadowWrapper = newElement('t', 'shadow-wrapper-' + this._opts.placement);
                    this._html.shadowFrame = newElement('t', 'frame', 'shadow-frame');
                ...
                this._html.contentWrapper = newElement('t', 'frame', 'content-wrapper');
                this._html.content = newElement('content', 't');
                ...
                if (this._opts.showCloseButton) {
                    if (this._opts.closeButtonMarkup) {
                        ...
                        applyCss(this._html.closeButton, ['close-button', 't']);
                    }
