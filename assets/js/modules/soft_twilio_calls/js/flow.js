        ...
        save: function (event, success) {
            ...
            $('#instances .flow-instance').each(function (index) {
                ...
                function () {
                    ...
                    $.ajax({
                        url: OpenVBX.home + '/settings/integrations/twilio/save/' + Flows.id,
                        data: params,
                        dataType: 'json',
    ...
    drop: {
        add: function (event, ui) {
            var afterDrop = function () {
                ...
                var icon = $(ui.draggable).data('applet_icon');
                $('input', event.target).attr('value', link.href.replace(/^\//, ''));
                var plugin_path = '/assets/OpenVBX/plugins/' + link.type.replace(/---/, '/applets/');
                $('.item-body', event.target).html('<a class="item-box" href="#flowline'
                    + link.href
                    + '"><div class="'
                    + link.type
                    + '-icon applet-icon ' + icon + '"><span class="replace">'
                    + link.type
...
Flows.initialize = function () {
    ...
    $(document).on('click', '.navigate-away, .util-menu a', function (e) {
    ...
    $(document).on('click', '.timing-timerange-wrap a', function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    $(document).on('click', '#flow-rename, .flow-name, #flow-rename-cancel', function (e) {
        e.stopPropagation();
        e.preventDefault();
        var $this = $(this);
    ...
    $(document).on('hover', '.flow-name', function () {
        $('#flow-rename').show();
    }, function () {
        $('#flow-rename').hide();
    });
    ...
    $(document).on('show', '.flow-instance', Flows.events.flow.shown);
    $(document).on('hide', '.flow-instance', Flows.events.flow.hidden);
    $(document).on('click', '.flow-instance', Flows.events.flow.select);
    $(document).on('set-name', '.flow-instance', Flows.events.flow.setName);
    $(document).on('click', '.flow-instance .close-flow-instance', Flows.events.flow.close);
    $(document).on('click', '.flow-instance .flowline-item-remove', Flows.events.drop.remove);
    ...
    $(document).on('submit', '.vbx-form', preventDefault);
    $(document).on('change', '.vbx-form :input, .flow-name-edit :input', function () {
        Flows.modified = true;
    });
