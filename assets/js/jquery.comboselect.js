...
(function ($) {
    jQuery.fn.comboselect = function (settings) {
        ...
        $('select.csright').dblclick(function () {
            //alert($(this).val());
            // alert($(this).attr('id').replace('_right',''));
            var ID = $(this).attr('id').replace('_right', '');//alert(ID);
            // alert( $('#' + ID).find('option[value="'+ this +'"]').attr('selected'));
            $('#' + ID).find('option[value="' + $(this).val() + '"]').removeAttr('selected');
