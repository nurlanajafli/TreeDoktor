...
$(function () {
    ...
    SP.functions.startWebSocket = function (worker) {
        ...
        var wsaddress = 'wss://' + baseUrl.replace(/^https:\/\//i, '').replace(/^http:\/\//i, '').replace(/\/+$/, '') + '/wss/?worker_sid=' + worker_sid;
        ...
        ws.checkerCallback = function(result) {
            ws.send(JSON.stringify({worker_sid:worker_sid, method:'imHere'}));
        }
            ...
            SP.worker.activities.fetch(
                function(error, activityList) {
                    ...
                    /*setTimeout(function(){
                        ...
                        SP.functions.notReady().then(function() {
                            SP.functions.ready();    
                        });
            ...
            if(data.workers.length)
            {
                ...
                $.each(data.workers, function(key, val){
                    if(val.sid != worker_sid)
                    {
                        ...
                        var link = `<li>
                            <a href="#" class="` + agentClass + ` clearfix" data-worker_sid="` + val.sid + `" data-contact_uri="` + agentAttributes.contact_uri + `">
                                <span class="pull-left agent-name">` + val.friendlyName + `</span>
        ...
        ws.getWorkersStatusesCallback = function(data) {
            $.each(data, function(key, worker){
                if(worker.sid != worker_sid)
                {
                    var attributes = JSON.parse(worker.attributes);
                    
                    var workerClass = worker.available ? 'callToAgent' : 'disabled';
                    var workerIconClass = worker.available ? 'text-success' : 'text-danger';
                    var html = `<li>
                                    <a href="#" class="` + workerClass + ` clearfix" data-worker_sid="` + worker.sid + `" data-contact_uri="`+attributes.contact_uri+`">
                                        <span class="pull-left agent-name">` + worker.friendlyName + `</span>
                                        <i class="fa fa-circle text-xs pull-right m-t-xs ` + workerIconClass + `"></i>
                                    </a>
                                </li>`;
                    $('.agents-status .dropdown-menu').append(html);
                }
            });
        };
        ...
        ws.getQueueCounterCallback = function(data) {
            ...
            var template = $.templates('#in-queues-list-empty-tmp');
            ...
            if(data.tasks){
                ...
                var template = $.templates('#in-queues-list-tmp');
                var htmlOutput = template.render(response);
            }
            $('#in-queues-list').html(htmlOutput);

            var template1 = $.templates('#queues-count-tmp');
            var htmlOutput1 = template1.render([{"queuesize":data.queuesize}]);
            $('#queues-count-result').html(htmlOutput1);            
            ...
            if (data.stats) {
                ...
                if(data.tasks.length)
                {
                    ...
                    $.each(data.tasks, function(key, val){
                        ...
                        if(val.client)
                            clientLink += ' - <a href="#" class="clientLink p-n text-ul" data-clientId="' + val.client.client_id + '" style="display: inline-block;">' + val.client.client_name + '</a>';
                        $("#team-status .dropdown-menu").append('<li style="white-space: nowrap;">' + (key + 1) + ') ' + val.caller + clientLink + '</li>');
                    });
                }
                else
                {
    ...
    SP.functions.attachAnswerButton = function (conn) {
        $("#action-buttons > button.answer").removeClass('inactive').addClass("active");
    }

    SP.functions.detachAnswerButton = function () {
        Twilio.Device.instance.soundcache.stop('incoming');
        $("#action-buttons > button.answer").removeClass('active').addClass("inactive");
    }
    ...
    $('.agents-status').on('shown.bs.dropdown', function () {
        ...
        $('.agents-status .dropdown-menu li').remove();
    });
    ...
    $("#action-buttons > button.hangup").click(function () {
        ws.send(JSON.stringify({worker_sid:worker_sid, method:'unsetCallSession'}));
        SP.active_call = false;
    ...
    SP.connection_loss = function(){
        //$.post(baseUrl + 'client_calls/restoreCall/' + worker_sid, function(){});
        Twilio.Device.activeConnection()._onHangup(Twilio.Device.activeConnection());
        ws.send(JSON.stringify({worker_sid:worker_sid, method:'restoreCall'}));
    }
    ...
    Twilio.Device.disconnect(function (conn) {
        ...
        //console.log(Twilio.Device.activeConnection());
        //console.log(SP.active_call);

        setTimeout(function(){
            if(Twilio.Device.activeConnection()==undefined || SP.active_call==false || Twilio.Device.activeConnection()._status == 'closed')
            {
                SP.currentCall = null;
    ...
    Twilio.Device.connect(function (conn){
        ...
        $.cookie('call_sid', conn.parameters.CallSid, { expires: date });*/
        ...
        if(Twilio.Device.activeConnection()!==undefined && Twilio.Device.activeConnection()._direction=="OUTGOING"){
            ...
            if(Twilio.Device.activeConnection()!=undefined && !isNaN(parseInt(Twilio.Device.activeConnection().options.callParameters.From)))
                ws.send(JSON.stringify({worker_sid:worker_sid, method:'setCallSession', params:{'call_sid':call_sid,'task_sid':task_sid}}));
        }
    });
    ...
    SP.functions.notReady = function () {
        ...
        //'WAb59015c8ffd0c47bb1f812a8c06c38d6';
    ...
    SP.functions.ready = function () {
        ...
        //if(reservation.reject!=undefined && reservation.reservationStatus!='accepted' && reservation.reservationStatus!='rejected')
        //   reservation.reject();

        Asid = SP.actvities.ready; 
        //'WAfc57c478bd7cea19883d8908c85b0f6b';
		//if(worker_sid == 'WK360ac12f67055ab09fbbec57a079069e' || worker_sid == 'WK4ecc2746fd3180d02cda96ace0335594') 
