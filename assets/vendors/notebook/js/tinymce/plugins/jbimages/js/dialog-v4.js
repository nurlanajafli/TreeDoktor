...
var jbImagesDialog = {
	...
	uploadFinish : function(result) {
		...
		else
		{
			...
			var w = this.getWin();
			tinymce = w.tinymce;
			var sub = result.filename.substr(-3);
			
			if(sub == 'jpg' || sub == 'png' || sub == 'jpeg' || sub == 'gif' || sub == 'JPG' || sub == 'PNG' || sub == 'JPEG' || sub == 'GIF')
			{
				tinymce.EditorManager.activeEditor.insertContent('<img src="' + result.filename +'">');
			}
			else
				tinymce.EditorManager.activeEditor.insertContent('<a href="' + result.filename + '" "target="_blank"">' + result.name + '</a>');
