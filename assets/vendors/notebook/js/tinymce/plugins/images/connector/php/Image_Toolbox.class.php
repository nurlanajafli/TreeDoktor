...
class Image_Toolbox {
	...
	function _addImage($argc, $args) {
		if (($argc == 2 || $argc == 3) && is_int($args[0]) && is_int($args[1]) && (is_string($args[2]) || !isset($args[2]))) {
			...
			$this->_img['operator'] = $this->_loadFile($args[0]);
                        if(isset($this->_img['operator']['type']) && $this->_img['operator']['type']!='FILE') // fix
	...
	function _loadFile($filename) {
		if (file_exists($filename)) {
			...
			if (!$info)
			{
                            //trigger_error($this->_error_prefix . 'Not an image ('.$filename.').', E_USER_ERROR); return null;
                            $filedata=array( // fix
                                'width'=>NULL,
                                'height'=>NULL,
                                'bias'=>NULL,
	...
	function newOutputSize($width, $height, $mode = 0, $autorotate = false, $bgcolor = '#000000') {
		...
		$dummy = $functionname($this->_img['target']['width'] + 1, $this->_img['target']['height'] + 1);
                if(isset($this->_img['target']['width']['typr']) && $this->_img['target']['width']['type']!='FILE')
