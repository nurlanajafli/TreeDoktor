...
$(document).ready(function () {
    ...
    $('#sendSMS').click(function (e) {
        e.preventDefault();
        $.post(OpenVBX.home + "/messages/sms/" + $('#sms-messageid').val(), {
            to: $('#sms-to-phone').val(),
            from: $('#sms-from-phone').val(),
            content: $('#message-to-send').val(),
            media_urls: $('#sms-media-url').val()
        }, function (data) {
        ...
        setTimeout(function () {
            getSmsHistory($('#sms-conversationid').val());
        }, 1500);
    });

    $(document).on('click', '.quick-call-button', function (e) {
        e.preventDefault();
