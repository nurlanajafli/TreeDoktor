...
<table class="table" id="tbl_Estimated">
	...
	<?php if(isset($estimators_mhr_rate) && !empty($estimators_mhr_rate)) : ?>
		...
		<?php foreach($estimators_mhr_rate as $key=>$report_row) : ?>
			<tr class="bg-<?php if($report_row->estimator_mhr_return2 < GOOD_MAN_HOURS_RETURN) : ?>danger<?php elseif($report_row->estimator_mhr_return2 > GOOD_MAN_HOURS_RETURN && $report_row->estimator_mhr_return2 < GREAT_MAN_HOURS_RETURN) : ?>warning<?php else : ?>success<?php endif; ?>">
				<td>
					<strong>
