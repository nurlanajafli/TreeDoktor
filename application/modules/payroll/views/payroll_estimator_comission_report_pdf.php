...
<div style="position: absolute; left: 3mm; width: 100%;">
    <?php foreach ($payrollWorkorders as $w => $week): ?>
	<table width="100%" class="report-page-table" autosize="1" style="overflow: wrap">
		<thead>
			<tr>
				<?php foreach ($week as $day => $data) : ?>
					<?php if(isset($week[$day]))  : ?>
					<th class="text-center bg-light b-r b-b" width="<?php echo (1750 / count($week)); //countOk ?>px" style="background: #f1f1f1;color: #717171; border-top: 1px solid #cfcfcf; line-height: 12px!important;"><?php echo date('D d-M-y', strtotime($day)); ?></th>
					<?php endif; ?>
				<?php endforeach; ?>
			</tr>
		</thead>
                        ...
                        <tr>
                            <td class="bg-dark text-center" style="line-height: 15px; <?php echo $strikeStyle; ?>">
                                ...
                                <span<?php if($workorderDamages) : ?> class="text-danger"<?php endif; ?>>
                                    AVG MHR:
                                    <strong>
                                    <?php if(!(int)$workorder['scheduled_wo_time']) $workorder['scheduled_wo_time'] = 1; ?>
