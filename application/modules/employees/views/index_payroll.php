...
<section class="padder">
        ...
        <div class="panel panel-default p-n"
            ...
            <header class="panel-heading"><?php echo isset($employee) ? $employee->emp_name : '' ?>
                <div class="pull-right" style=" ">
                    <a href="<?php echo base_url('employees/payroll/' . $employee_id . '/' . ($payroll->payroll_id - 1)); ?>"
                       class="btn btn-sm btn-info"><< Previous Weeks</a>
                    <a href="<?php echo base_url('employees/payroll/' . $employee_id); ?>"
                       class="btn btn-sm btn-default" id="this-week">Current Weeks</a>
                    <a href="<?php echo base_url('employees/payroll/' . $employee_id . '/' . ($payroll->payroll_id + 1)); ?>"
                       class="btn btn-sm btn-info">Next Weeks >></a>
                </div>
            ...
            <?php $this->load->view('payroll_employee_workspace'); ?>
        </div>
    <?php endif; ?>
    ...
    <script>
        ...
        function _after_insert(resp) {
            ...
            if (localStorage.getItem('range_start') && localStorage.getItem('range_end')) {
                location.reload();
                return true;
            }
        ...
        function _reload_workspace() {
            ...
            $.post(baseUrl + 'employees/payroll/<?php echo $employee_id; ?>/<?php echo $payroll_id; ?>', function (resp) {
                $('#workspace').html(resp);
                $('.employeeLunch, .employeeRate, .employeeDeductions').popover();
                $('.employeeLunch, .employeeRate, .employeeDeductions').on('click', function (e) {
                    $('.employeeLunch, .employeeRate, .employeeDeductions').not(this).popover('hide');
...
<?php if (isset($payroll)) : ?>
    <?php foreach ($weeks as $key => $week) : ?>
        <?php for ($i = $week['week_start_time']; $i <= $week['week_end_time'] + 3600; $i += 86400) : ?>
            <?php $key = date('Y-m-d', $i + 3600); ?>
            <?php if (!isset($pdf)) : ?>
                <?php $this->load->view('payroll_change_employee_time',
                    array(
                        'time' => $key,
                        'logins' => isset($worked_data[$key]->mdl_emp_login) ? $worked_data[$key]->mdl_emp_login : array(),
