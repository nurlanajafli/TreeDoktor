...
<script>
	...
	$(document).ready(function(){
		$('.scrollable.p-sides-15').scroll(function() {
			if(load)
				return false;
			if($('.scrollable.p-sides-15').scrollTop() + 1120 >= $('.scrollable.p-sides-15')[0].scrollHeight) {
				...
				$.post(baseUrl + 'reports/ajax_load_estimates', {offset:offset, estimator_id:estimator_id}, function(resp){
					if(resp.status == 'ok')
					{
						if(!resp.estimators_files.length)
							$('.scrollable.p-sides-15').unbind('scroll');
						offset += 100;
