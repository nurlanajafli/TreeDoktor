...
<section class="scrollable p-sides-15">
	...
	<section class="panel panel-default">
		<header class="panel-heading">Filter
			<div class="pull-right" style="margin-top:-14px;">
				<form id="dates" method="post" action="<?php echo base_url('reports/client_payments/'); ?>" class="input-append m-t-xs">
					<label>
					...
					<label>
						<input name="from" class="datepicker form-control date-input-client from" type="text" readonly
							   value="<?php if (isset($from)) : echo date('Y-m-d', strtotime($from));
					...
					<label>
						<input name="to" class="datepicker form-control date-input-client to" type="text" readonly
							   value="<?php if (isset($to)) : echo date('Y-m-d', strtotime($to));
				...
				<tbody>
					...
					<?php if(isset($filter) && !empty($filter) && $filter) : ?>
						<?php foreach($filter as $key=>$val) : ?>
							<tr>
								<td><strong><?php echo money($val['payment_account']);  ?></strong></td>
	...
	$(document).ready(function(){
		$('#dates').submit(function(){
			...
			url = baseUrl + 'reports/client_payments/' + $('.from').val() + '/' + $('.to').val();
			if(amount != '')
				url += '/' + $.trim('am' + amount);
			location.href = url;
			return false;
		...
		$(document).on("click", '.change_checked', function(){
			...
			$.post(baseUrl + 'reports/ajax_change_payments', {id : id, checked : checked}, function (resp) {
				if (resp.status == 'ok')
				{
		...
		$('.payment_account').on("change", function(){
			...
			$.post(baseUrl + 'reports/ajax_change_account', {id : id, data_id : data_id}, function (resp) {
