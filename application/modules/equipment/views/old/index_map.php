...
<section class="scrollable p-sides-15 mapper" style="top: 9px;">
	...
	<header class="panel-heading">Filter
		<div class="pull-right" >
			<form id="dates" method="post" action="<?php echo base_url('equipments/map'); ?>" class="input-append m-t-xs inline" >
				<select name="truck" class="form-control truck inline" style="width: 106px;">
					<option value="">Equipment</option>
					<?php foreach($items as $k=>$v) : ?>
						<option <?php if(isset($truck) && $truck->item_id == $v->item_id) : ?>selected="selected"<?php endif; ?> value="<?php echo $v->item_code;?>">
...
<script>
	...
	$(document).ready(function(){
		...
		$('#dates').submit(function(){
			url = baseUrl + 'equipments/map/' + $('.date').val() + '/' + $('.truck').val();
			 
			location.href = url;
			return false;
		});
		$('.reset').on('click', function(){
			location.href = baseUrl + 'equipments/map';
		});
