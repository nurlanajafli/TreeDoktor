...
<pagebreak>
	...
	<div class="des_1">The invoice is due upon receipt and interest will begin to accrue on any unpaid balance 7 days after the invoice date, at a rate of 2% per month. If Customer has provided a credit card number to <?php echo $this->config->item('company_name_long'); ?>, Customer authorizes <?php echo $this->config->item('company_name_long'); ?> to charge unpaid amounts to such a credit card when due. Payment of any amount shall not be construed as acceptance by Customer of defective Work, and Customer agrees not to withhold any amount invoiced in accordance with this contract for any reason whatsoever. Failure by Customer to pay any amount within 7 days after the invoice date shall constitute a breach of this contract, and in addition to paying <?php echo $this->config->item('company_name_long'); ?> all amounts due hereunder, Customer shall indemnify <?php echo $this->config->item('company_name_long'); ?> for all costs incurred in collecting such unpaid amounts.
	</div>
