...
<head>
	...
	<link rel="stylesheet" href="<?php echo base_url(); ?>assets/css/global_invoice_pdf.css" type="text/css">

    <?php

    $default_img = 'assets/'.$this->config->item('company_dir').'/print/header2_short.png';
    $brand_id = get_brand_id(isset($estimate_data)?$estimate_data:[], isset($client_data)?$client_data:[]);
...
<body>
<div class="holder" style="position: absolute; top: 0; right: -45px;"><img class="top_logo" src="<?php echo base_url($invoice_logo); ?>" width="120"></div>
<?php if(isset($invoice_data) && $invoice_data) : ?>
<div class="estimate_number">
	<div class="estimate_number_1">Invoice Date</div>
<!--	<div class="estimate_number_2">--><?php //echo $invoice_data->date_created; ?><!--</div>-->
...
<div class="data">
	...
	<table cellpadding="0" cellspacing="0" border="0" class="client_table">
		...
		<?php if (element('cc_email', $client_contact, FALSE)) : ?>
			<tr>
				<td>Client Email:</td>
				<td><?php echo element('cc_email', $client_contact, ''); ?></td>
			</tr>
		<?php endif; ?>
		<?php if (isset($client_data->client_address) && isset($estimate_data->lead_address) && $client_data->client_address != $estimate_data->lead_address) : ?>
			<tr>
				<td>Job Site Location:</td>
				<td>
				<?php if(isset($estimate_data->lead_address)) : ?>
					<?php echo $estimate_data->lead_address . '' . " "; ?>
					<?php if(isset($estimate_data->lead_city)) : ?>
						<?php echo $estimate_data->lead_city; ?>
					<?php endif; ?>
		...
		<?php if (isset($client_data->client_name) && isset($client_contact['cc_name']) && $client_data->client_name != $client_contact['cc_name']) : ?>
			<tr>
				...
				<td><?php echo element('cc_name', $client_contact, ''); ?></td>
			</tr>
		<?php endif; ?>
	</table>
	<!-- CLIENT INFO -->
...
<div class="title">Partial Invoice Details</div>
<div class="data">
	<div class="min_height_350" <?php if($invoice_watermark): ?>style="background-image: url('<?php echo base_url($invoice_watermark); ?>')!important;"<?php endif; ?>>
        ...
        <div class="estimate_table">
                ...
                <?php foreach ($estimate_services_data as $numService => $service_data) : ?>
                    <div class="m-b-5" style="padding-top: 10px;">
                        ...
                        <div class="pull-left text-left p-b-5" style="text-align: justify;">
                            <strong><?php echo isset($service_data['estimate_service_ti_title']) && !empty($service_data['estimate_service_ti_title']) ? $service_data['estimate_service_ti_title'] : $service_data['service_name']; ?></strong>
                            <span style="width: <?php echo $priceColumnWidth; ?>px;">
                                                <?php if($service_data['is_product']) : ?>
                                                    (<?php echo $service_data['quantity']?> x <?php echo nl2br(money($service_data['cost'])); ?>)
                                                <?php endif ?>
                            </span>
                        </div>
                        <div class="clear"></div>
                                        ...
                                        <div class="pull-left text-left">
                                            ...
                                            <span style="width: <?php echo $priceColumnWidth; ?>px;">
                                                <?php if($bundle_record->is_product) : ?>
                                                    (<?php echo $bundle_record->quantity ?> x <?php echo nl2br(money($bundle_record->cost)); ?>  <?php echo str_replace(' ', '', nl2br(money($bundle_record->service_price)))?>)
                                                <?php else : ?>
                                                    (<?php echo nl2br(money($bundle_record->service_price)); ?>)
                                                <?php endif ?>
                                        </span>
            ...
            <!-- ESTIMATE INFO -->
		<table cellpadding="0" cellspacing="0" border="0" class="invoice_table" width="100%" height="100%">
            <?php if(is_array($estimate_services_data) && !empty($estimate_services_data)) : ?>
			<tr>
				<td valign="middle" align="right" class="small_arrow">Sum:</td>
				<td  valign="middle" align="right" class="p-30" width="130px"><?php echo money($estimate_data->sum_for_services); ?></td>
			</tr>
                <?php
                ...
                <?php if ((float)$estimate_data->discount_total) : ?>
                    <tr>
                ...
                <?php if ($estimate_data->estimate_hst_disabled != 1 && $estimate_data->estimate_tax_name && $estimate_data->estimate_tax_value) : ?>
                    <tr>
                        <td valign="top" align="right"
                            ...
                            <?php echo /*$estimate_data->estimate_tax_name . */ 'Tax: ' . round($estimate_data->estimate_tax_value, 2) . '%'; ?>:
                        </td>
                        <td valign="middle"
                ...
                <?php endif; ?>
                <tr>
                    <td valign="top" align="right" class="small_arrow"><strong>Subtotal:</strong></td>
            ...
            <?php else : ?>
                ...
                <tr><td colspan="2"> </td></tr>
                <tr>
                    <td colspan="2" align="center" valign="middle" height="100%;"><span style="text-align: center; font-size: 30px;">NO DATA</span></td>
                </tr>

            <?php endif; ?>
			<?php /*if(number_format((float)$total, 2, '.', '') <= 0) : ?>
			<tr>
...
<div class="thank_you">Thank you for your business!</div>
...
<div class="address" style="position: absolute; bottom: 15px; right: 0; left: 0;">
	...
	<?php endforeach; ?>
	<?php endif; ?>
	<?php if(isset($estFiles) && $estFiles && count($estFiles)) : ?>
	<pagebreak>
