...
namespace application\modules\settings\integrations\twilio\libraries\Services;
...
use Services_Twilio_HttpStream;
use Services_Twilio_Resource;
use Services_Twilio_Rest_Accounts;
...
class Services_Twilio extends Services_Twilio_Resource
{
    ...
    protected $version;
    protected $versions = ['2008-08-01', '2010-04-01'];

    /**
     * Services_Twilio constructor.
     * @param $sid
     * @param $token
     * @param null $version
    ...
    public function __construct(
        ...
        if (null === $_http) {
            if (!in_array('openssl', get_loaded_extensions())) {
                throw new \Exception("The OpenSSL extension is required but not currently enabled. For more information, see http://php.net/manual/en/book.openssl.php");
            }
    ...
    {
        $response = call_user_func_array($callable, [$uri]);
    ...
    public function retrieveData(
        ...
        return $this->_makeIdempotentRequest([$this->http, 'get'], $uri, $this->retryAttempts);
    }
        ...
        foreach ($queryData as $key => $value) {
            ...
            if (is_array($value)) {
                ...
                foreach ($value as $value2) {
                    ...
                    // Recurse
                    $query .= self::buildQuery([$key => $value2], $numericPrefix);
                }
            } else {
                // Add an arg_separator if needed
                if ($query !== '') {
