...
class Services_Twilio_Twiml
{
    ...
    public function __call($verb, array $args)
    {
         ...
         */
        $decoded = html_entity_decode($noun, ENT_HTML5, 'UTF-8');
        $normalized = htmlspecialchars($decoded, ENT_HTML5, 'UTF-8', false);

        $child = empty($noun)
            ? $this->element->addChild(ucfirst($verb))
            : $this->element->addChild(ucfirst($verb), $normalized);
        if (!empty($attrs) && !is_null($attrs)) {
