...
class cfb {
	...
	public function getStreamIdByName($name, $from = 0) {
		for($i = $from; $i < count($this->fatEntries); $i++) {//countOk
	...
	private function readDIFAT() {
		...
		while($this->DIFAT[count($this->DIFAT) - 1] == self::FREESECT)//countOk
	...
	private function readFATChains() {
		...
		for ($i = 0; $i < count($this->DIFAT); $i++) {//countOk
	...
	private function readDirectoryStructure() {
		...
		while($this->fatEntries[count($this->fatEntries) - 1]["type"] == 0)//countOk
