...
class Googlemaps
{
	...
	//old key AIzaSyBDUPTmUYuYnv9r8d6-TXanSefGZclfKTw
	...
	function __construct($config = array())
	{
		...
		$this->apiKey = $this->ci->config->item('gmaps_key');
	...
	function add_marker($params = array())
	{
		...
		$marker = array();
		$this->markersInfo['marker_' . count($this->markers)] = array();//countOk
		...
		$marker_id = count($this->markers);//countOk
		if (trim($marker['id']) != "") {
			$marker_id = $marker['id'];
		}
		...
		if ($marker['position'] != "") {
			...
			';
				...
				//var_dump($lat_long);
				if(isset($lat_long[0]) && isset($lat_long[1]) && $lat_long[0] && $lat_long[1])
					$marker_output .= '
			var myLatlng = new google.maps.LatLng(' . $lat_long[0] . ', ' . $lat_long[1] . ');';
				else
					$marker_output .= '
			//console.log("' . $lat_long[0] . ' ' . $lat_long[1] . '", "' . $lat_long[2] . '");';
		...
		if (isset($marker['stump_id']) && $marker['stump_id'] != "") {
			$marker_output .= ',
				stump_id: ' . $marker['stump_id'];
		}
		if (!$marker['clickable']) {
			$marker_output .= ',
				clickable: false';
		...
		if ($marker['infowindow_content'] != "") {
			...
			';
				...
				$marker_output .= 'iw.setContent(this.get("content"));';
				$marker_output .= 'iw.open(' . $this->map_name . ', this);';
			}
			$marker_output .= '
			});
			';
		...
		if ($marker['infowindow_arr_content'] != "") {

			// Escape any quotes in the event that HTML is being added to the infowindow
			$marker['infowindow_content'] = str_replace('\"', '"', $marker['infowindow_arr_content']);
			$marker['infowindow_content'] = str_replace('"', '\"', $marker['infowindow_arr_content']);

			$marker_output .= '
			marker_' . $marker_id . '.set("content", "' . $marker['infowindow_arr_content'] . '");
			...
			';
				...
				$marker_output .= 'iwArr['.$marker_id.'] = new google.maps.InfoWindow();';
				$marker_output .= 'iwArr['.$marker_id.'].setContent(this.get("content"));';
				$marker_output .= 'iwArr['.$marker_id.'].open(' . $this->map_name . ', this);';
			}
			$marker_output .= '
			});
			';
	...
	function add_polyline($params = array())
	{
		...
		if (count($polyline['points'])) {//countOk

			$polyline_output .= '
				var polyline_plan_' . count($this->polylines) . ' = [';//countOk
			$i = 0;
			$lat_long_output = '';
			...
			$polyline_output .= '
				var polyline_' . count($this->polylines) . ' = new google.maps.Polyline({
    				...
    				strokeWeight: ' . $polyline['strokeWeight'];//countOk
	...
	function add_polygon($params = array())
	{
		...
		if (!empty($polygon['points'])) {

			$polygon_output .= '
				var polygon_plan_' . count($this->polygons) . ' = [';//countOk
			$i = 0;
			$lat_long_output = '';
		...
		$polygon_output .= '
			...
			polygons.push(polygon_' . count($this->polygons) . ');
		';//countOk
	...
	function create_map($includeJs = TRUE)
	{
		...
		if($includeJs) {
			$this->output_js .= '
			<script type="text/javascript" src="' . $apiLocation . '&libraries=places"></script>';
		...
		$this->output_js_contents .= '
			...
			var iwArr = []; //new google.maps.InfoWindow(); // Global declaration of the infowindow
		...
		$this->output_js_contents .= 'function initialize_map() {
		...
		if (!empty($mapTypeControlOptions)) {
			$this->output_js_contents .= ',
		...
		if ($this->drawing) {
			...
			google.maps.event.addListener(drawingManager, "overlaycomplete", function(event) {
				...
				';
			if (!empty($this->drawingOnComplete)) {
		...
		if ($this->places) {
			...
			if (($placesLocationSet || $this->placesLocation != "") || !empty($this->placesTypes) || $this->placesName != "") {
			...
			if ($this->placesAutocompleteInputID != "") {
				...
				if (!empty($this->placesAutocompleteTypes)) {
		...
		if ($this->loadAsynchronously) {
			...
			function loadScript() {
				var script = document.createElement("script");
  				script.type = "text/javascript";
  				script.src = "' . $apiLocation . '&callback=initialize_map";
	...
	function get_lat_long_from_address($address, $attempts = 0)
	{
		...
		$data_location .= "maps.google.com/maps/api/geocode/json?address=" . urlencode(utf8_encode($address)) . "&key=" . $key;
		if ($this->region != "" && strlen($this->region) == 2) {
			$data_location .= "&region=" . $this->region;
		}
		//$data = @file_get_contents($data_location);
		$curl = curl_init();
		curl_setopt($curl, CURLOPT_URL, $data_location);
