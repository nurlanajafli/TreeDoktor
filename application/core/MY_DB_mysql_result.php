...
class MY_DB_mysql_result extends CI_DB_mysql_result
{
    ...
    public function get_foreigners($res)
    {
        ...
        $sql = "SELECT * FROM (
				...
				) AS foreigns_view WHERE TABLE_NAME='".$this->base_table."' AND (COLUMN_NAME='".implode("' OR COLUMN_NAME='", $fields)."')";
        ...
        foreach($result as $ref)
        {
			if (!is_object($res))
			{
				$field_name = $ref->REFERENCED_TABLE_NAME;
				$field = $ref->COLUMN_NAME;
				$CI->db->from($field_name);
				foreach($res as &$row){
					$vals[] = $row->$field;
				}
				$CI->db->where_in($ref->REFERENCED_COLUMN_NAME, $vals);
				$query = $CI->db->get();
				$records = $query->result();
				foreach($records as $rec) {
					$sortedRef[$rec->$field] = $rec;
				}
				foreach($res as &$row){
					$row->$field_name = $sortedRef[$row->$field];
				}
				/*foreach($res as &$row){
					$field_name = $ref->REFERENCED_TABLE_NAME;
					$field = $ref->COLUMN_NAME;
					$CI->db->from($field_name);
					$CI->db->where($ref->REFERENCED_COLUMN_NAME,$row->$field);
					$query = $CI->db->get();
				...
				$field_name = $ref->REFERENCED_TABLE_NAME;
				$CI->db->from($field_name);
				$CI->db->where($ref->REFERENCED_COLUMN_NAME,$res->$field);
				$query = $CI->db->get();
