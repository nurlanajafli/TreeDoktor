...
class JR_Model extends CI_Model
{
    ...
    public function get_all()
    {
        ...
        /***FIX BY RUSLAN GLEBA 20.03.2015***/
        if (!$result)
            return FALSE;
        /******/
    ...
    public function insert_many($data, $skip_validation = FALSE)
    {
        ...
        foreach ($data as $key => $row) {
            $ids[] = $this->insert($row, $skip_validation, ($key == count($data) - 1)); //countOk
        }
        return $ids;
    }
        ...
        foreach ($this->has_many as $key => $value) {
            ...
            if (in_array($relationship, $this->_with)) {
                ...
                if (is_object($row)) {
                    ...
                    if (isset($options['from_field']))
                        $valueFrom = $row->{$options['from_field']};
                    $row->{$relationship} = $this->{$relationship . '_model'}->get_many_by($options['primary_key'], $valueFrom);
                } else {
                    $valueFrom = $row[$this->primary_key];
                    if (isset($options['from_field']))
                        $valueFrom = $row[$options['from_field']];
    ...
    {
        return (int)$this->_database->query("SELECT AUTO_INCREMENT FROM information_schema.TABLES WHERE
			TABLE_NAME = '" . $this->_database->dbprefix . $this->_table . "' AND
			TABLE_SCHEMA = '" . $this->_database->database . "'")->row()->AUTO_INCREMENT;
        /*return (int) $this->_database->select('AUTO_INCREMENT')
            ->from('information_schema.TABLES', TRUE)
            ->where('TABLE_NAME', $this->_table)
            ->where('TABLE_SCHEMA', $this->_database->database)->get()->row()->AUTO_INCREMENT;*///BY GLEBA RUSLAN - trouble with tables prefix
    }
    ...
    {
        if (count($params) == 1 && is_array($params[0])) { //countOk
        ...
        } else if (count($params) == 1) { //countOk
            $this->_database->where($params[0]);
        } else if (count($params) == 2) { //countOk
            if (is_array($params[1])) {
                $this->_database->where_in($params[0], $params[1]);
            } else {
                $this->_database->where($params[0], $params[1]);
            }
        } else if (count($params) == 3) { //countOk
    ...
    /*function uploadFile($dir, $name, $field = 'file', $types = '*', $width = FALSE, $height = FALSE)
    {
        ...
        if (!$this->upload->do_upload($field))
        {
            ...
            $file['filepath'] = $dir . '/' . $config['file_name'];
            $file['filename'] = $config['file_name'];
            @chmod($file['filepath'], 0777);

            if($width && $height)
            {
                $config['image_library'] = 'gd2';
                $config['source_image']	= $file['filepath'];
    ...
    public function join($tableName, $rule, $type = 'inner')
    {
        $this->_database->join($tableName, $rule, $type);
        return $this;
    }
