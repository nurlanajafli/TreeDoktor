...
class Queue_Controller extends MX_Controller
{
    /**
     * Debug mode
     *
    ...
    function __construct()
    {
        ...
        // register signals before anything else
        pcntl_signal(SIGTERM, array(&$this, 'sig_handler' ));
        pcntl_signal(SIGQUIT, array(&$this, 'sig_handler' ));

        parent::__construct();

        $this->phpCommand = config_item('php_command') !== null ? config_item('php_command') : $this->phpCommand;
    ...
    public function set_terminating_on ($signo) {
      $this->_log("Queue Signal Hangler - Signal Received (".$signo."). Terminating.");
    ...
    public function sig_handler ($signo) {
        switch ($signo) {
            case SIGTERM:
                ...
                $this->set_terminating_on($signo);
                break;
            case SIGQUIT:
                // mainly received when when docker stop issued
                $this->set_terminating_on($signo);
                break;
        }
    }

    public function signal_workers () {
      if ( $this->terminating ) {
        $msg = "Queue Listener - sending kill signal to Worker PID(s): ";
        foreach ($this->_pidStack as $id => $pid) {
          $msg = $msg . $pid . " ";
          $output = shell_exec('kill -s TERM '.$pid);
        }
        $this->_log($msg);
      }
    }
    ...
    public function listen()
    {
        ...
        while (true) {
            ...
            //$this->_log("Has event: " . (int)$hasEvent);
            ...
            if ($hasEvent) {
                ...
                //$this->_log('$this->workerMaxNum <= $workerCount: ' . ($this->workerMaxNum <= $workerCount));
                ...
                if ($this->workerMaxNum <= $workerCount) {
                    ...
                    if ($this->workerHeathCheck) {
                        foreach ($this->_pidStack as $id => $pid) {
                            if($pid === NULL) {
                                $this->_log("Queue Listener - Worker health check restart: Missing #{$id} (PID: {$pid})");
                                $r = $this->_workerCmd($workerCmd, $id);
                                break;
                            }
                            $isAlive = $this->_isPidAlive($id, $pid);
                            if (!$isAlive) {
                                $this->_pidStack[$id] = NULL;
            ...
            foreach ($this->_pidStack as $id => $pid) {
                ...
                if (!$isAlive) {
                    $this->_pidStack[$id] = NULL;
                    $this->_log("Queue Listener - Worker Finished: Missing #{$id} (PID: {$pid})");
                } else {
                    $stillWork = TRUE;
                }
            }
            //$this->_log('$workingFlag: ' . (int)$workingFlag);
            // The end of assignment (No more work), close the assignment
            if ($workingFlag && !$stillWork) {
                $workingFlag = false;
                $workerCount = 0;
                // Clear worker stack
                $this->_pidStack = [];
            ...
            if ( !$workingFlag || ($workingFlag && !$stillWork)) {
              if ( $this->terminating ) {
                  $this->_log("Queue Listener - Exiting");
                  break;
                }
            }
            // Idle
    ...
    public function work($id=1)
    {
        ...
        while ($this->handleWork($this->_staticWork, $pid)) {
            // Sleep if set
            if ($this->workerSleep) {
    ...
    protected function _workerCmd($workerCmd, $workerCount)
    {
        ...
        stream_set_blocking($pipe[2], 0);*/
        // Find out worker command's PID
        $this->pipe[$workerCount] = $pipe;
        $status = proc_get_status($process);
        $pid = $status['pid'] + 1;
        // Stack workers
        $this->_pidStack[$workerCount] = $pid;
        $this->_processes[$workerCount] = $process;
        //$message = stream_get_contents($pipe[1]);
        //fclose($pipe[1]);
        // Close
        //proc_close($process);
        // Log
     ...
     * @return boolean
     */
    protected function _isPidAlive($id, $pid)
    {
        $status = proc_get_status($this->_processes[$id]);
        if (!$status['running']) {
            foreach ($this->pipe[$id] as $sid => $pipe) {
                if ($sid == 0) {
            ...
            foreach ($this->pipe[$id] as $pipe) {
                fclose($pipe);
            }
            proc_close($this->_processes[$id]);
        }
