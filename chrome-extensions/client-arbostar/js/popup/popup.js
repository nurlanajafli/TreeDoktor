...
var SP = {
    ...
    updateAgentStatusText: function (statusCategory, statusText, inboundCall, outboundCall) {
        ...
        if (inboundCall == true) {
            ...
            let clientName = '';
            if (Object.keys(bgp.ctp.currentTask).length > 0) {
                clientName = bgp.ctp.currentTask.attributes.clientName;
                let clientId = bgp.ctp.currentTask.attributes.clientId;
                if (clientName !== undefined && clientId !== undefined) {
                    clientName = '<a target="_blank" href="' + baseUrl + '/' + clientId + '">' + clientName + '</a>';
                }
            }

			$('#call-number').html(bgp.Twilio.Device.activeConnection().options.callParameters.From + ' ' + clientName);
			$('#call-info').show();
        ...
        if (outboundCall == true) {
			...
			if (bgp.Twilio.Device.activeConnection().options.callParameters.To != undefined) {
                SP.callNumber = bgp.Twilio.Device.activeConnection().options.callParameters.To;
            }
			$('#call-number').text(SP.callNumber);
			$('#call-info').show();
        }
    ...
    setRingState: function () {
        ...
        console.log(SP.action_buttons[0].answer, 'answer');
...
$(document).on('click', "#action-buttons > button.call", function () {
    SP.active_call = true;
    SP.callNumber = $('#outgoing_call_to').val();
